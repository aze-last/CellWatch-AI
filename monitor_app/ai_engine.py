...
class MotionOptimizedEngine:
    ...
    def __init__(self, debug=False, sensitivity="medium", enable_yolo=True, **kwargs):
        ...
        # AI Models
        self.movenet = None
        self.yolo_custom = None    # best.pt (Knives)
        self.yolo_standard = None  # yolov8n.pt (Phones/Others)
        ...
        self.EDGES = [(0,1),(0,2),(1,3),(2,4),(0,5),(0,6),(5,7),(7,9),(6,8),(8,10),(5,6),(5,11),(6,12),(11,12),(11,13),(13,15),(12,14),(14,16)]
    ...
    def _setup_hardware(self):
        """Enable RTX 2050 GPU for both TF and Torch."""
        if TF_AVAILABLE:
            try:
                ...
                if gpus:
                    ...
                    CapstoneDebug.log("All", f"MoveNet: GPU Accelerated ({gpus[0].name})")
            except: pass
        if YOLO_AVAILABLE and torch.cuda.is_available():
            print("✅ YOLOv8: GPU Accelerated (CUDA)")
    ...
    def _load_models(self):
        ...
        if self.enable_yolo:
            ...
            print("Loading Custom YOLOv8 (Knife engine: best.pt)...")
            try:
            ...
            # 2. Load Standard Model (yolov8n.pt)
            print("Loading Standard YOLOv8 (Phone engine: yolov8n.pt)...")
            try:
                self.yolo_standard = YOLO('yolov8n.pt')
            except Exception as e:
                print(f"Standard YOLO load failed: {e}")
    ...
    def detect_motion(self, frame, camera_id):
        ...
        if camera_id not in self.bg_subtractors:
            self.bg_subtractors[camera_id] = cv2.createBackgroundSubtractorMOG2(history=300, varThreshold=30, detectShadows=False)
    ...
    def process_frame(self, frame, camera_id="0"):
        ...
        if self.enable_yolo and (self.yolo_custom or self.yolo_standard):
            ...
            try:
                ...
                for r in results:
                    for box in r.boxes:
                        conf = float(box.conf)
                        if conf > 0.83:
                            # Force the label "knife" for the custom model detections
                ...
                CapstoneDebug.log(cam_id, f"Custom YOLO Error: {e}")

        # 2. Standard Model -> CELLPHONE ONLY
        if self.yolo_standard:
            try:
                results = self.yolo_standard(res["frame"], verbose=False, imgsz=640, device=device)
                PHONE_CLASS_ID = 67
                for r in results:
                    for box in r.boxes:
                        cls_id = int(box.cls)
                        ...
                        # Use class ID (67) for brittle-free cellphone detection
                        if cls_id == PHONE_CLASS_ID and conf > 0.15:
                            detections_found.append({
                                "name": "cell phone",
                                "conf": conf,
                                "box": box.xyxy[0],
                ...
                CapstoneDebug.log(cam_id, f"Standard YOLO Error: {e}")
    ...
    def draw_skeleton(self, frame, kps, p_id):
        ...
        for p1, p2 in self.EDGES:
            if kps[p1,2] > 0.3 and kps[p2,2] > 0.3:
                cv2.line(frame, (int(kps[p1,1]*w), int(kps[p1,0]*h)), (int(kps[p2,1]*w), int(kps[p2,0]*h)), color, 2)
